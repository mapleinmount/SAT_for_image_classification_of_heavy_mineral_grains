...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/imagenet/imagenet_san10_pairwise.yaml',
                        help='config file')
    parser.add_argument('--trainset', type=str, help='test sets')
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    model = torchvision.models.vgg16(pretrained=False, progress=True, num_classes=args.classes)
    ...
    checkpoint = torch.load("/root/.cache/torch/hub/checkpoints/vgg16-397923af.pth")
    pth = checkpoint
    for key in list(pth.keys()):
        if key.split(".")[0] == "classifier":
            del pth[key]
    model.module.load_state_dict(pth, False)
    if args.weight:
    ...
    train_domain_folders = []
    val_domain_folders = []
    set_dict = {'C': "/deepo_data/GSP/heavy_minerals/data/Yangtze/train",
                'P': "/deepo_data/GSP/heavy_minerals/data/PumQu/train",
                'Y': "/deepo_data/GSP/heavy_minerals/data/YarlungZangbo/train"}
    for set_ in args.trainset:
        train_domain_folders.append(set_dict[set_])
        val_domain_folders.append(set_dict[set_].replace("train", "val"))

    # train_domain_folders = ["/deepo_data/GSP/heavy_minerals/data/Yangtze/train",
    #                         "/deepo_data/GSP/heavy_minerals/data/PumQu/train",]
    ...
    val_transform1 = transforms.Compose(
        [transforms.Resize((256, 256)), transforms.CenterCrop((224, 224)), transforms.ToTensor(), transforms.Normalize(mean1, std1)])
    val_transform2 = transforms.Compose(
        [transforms.Resize((256, 256)), transforms.CenterCrop((224, 224)), transforms.ToTensor(), transforms.Normalize(mean2, std2)])
